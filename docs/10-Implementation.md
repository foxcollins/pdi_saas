...
- **E11 analytics**: página `/app/analytics` con totales (conversaciones, leads, mensajes, sin respuesta, costo IA del mes), gráfico de tendencia diaria SVG/CSS puro sin librerías (conversaciones/mensajes/leads/sin respuesta), selector de rango 7/14/30 días, y desglose por canal, por estado de lead y por origen. `AnalyticsController` agrupa con RLS (aislamiento cross-tenant testado). Tests en `AnalyticsTest` (4 tests).
- **E12 billing básico**: `PlanService` centraliza límites por plan (docs, páginas, canales, mensajes IA mensual/diario, por minuto, tokens máx) con overrides por tenant (`tenants.settings.ai`); `AiUsageService` resuelve la política desde el plan; página `/app/billing` muestra plan actual, consumo vs límites (documentos, páginas, canales, mensajes IA, costo) y permite cambiar de plan (actualiza `tenants.plan_id` + `subscriptions` activa). `PlanSeeder` actualizado con límites IA por plan. Tests en `BillingTest` (4 tests). Stripe pendiente de fase siguiente.
- **Fix retrieval/RAG**: con proveedor fake, consultas legítimas devolvían 0 resultados porque el umbral `confidence_threshold` (0.16) filtraba los embeddings deterministas. `RetrievalService` ahora baja el umbral a `min(0.16, 0.10)` en modo fake y ejecuta un fallback por keywords (ILIKE sobre hasta 8 términos, score por nº de hits) cuando el vector search queda vacío y existen chunks. Tests en `KnowledgeRagTest` (fallback, vacío, cross-tenant).
- **Fix parser DOCX**: el extractor OOXML dejaba tags XML residuales (`standalone`, atributos, `w:t` anidados) en el texto extraído (p. ej. "resumen mis puesticos MVP.docx" de foxcode). `OfficeTextExtractor::extractDocx()` ahora limpia los tags restantes con regex, colapsa espacios en blanco y aplica `html_entity_decode`. Tests en `KnowledgeOfficeTest` (atributos y entidades).
